// ==== FeedHunger.org – Full React App with Firebase (Cleaned) ====
            ...
            <main>
                <section className="block">
                    ...
                    <form onSubmit={handleSubmit}>
                        <label>Goods<input name="goods" value={formData.goods} onChange={handleChange} required /></label>
                        <label>Price (HungerCoin)<input name="price" type="number" value={formData.price} onChange={handleChange} required /></label>
                        <label>Contact Info<input name="contact" value={formData.contact} onChange={handleChange} required /></label>
                        <label>Country<select name="country" value={formData.country} onChange={handleChange} required>
                            <option value="">Select</option>
                            {countries.slice(1).map(c => <option key={c} value={c}>{c}</option>)}
                        </select></label>
                        <label>About Your Production<textarea name="about" value={formData.about} onChange={handleChange} required /></label>
                ...
                <section className="block">
                    ...
                    <label>Filter by country<select value={selectedCountry} onChange={(e) => setSelectedCountry(e.target.value)}>{countries.map(c => <option key={c} value={c}>{c}</option>)}</select></label>
                    <div className="grid">
                        {filtered.map(item => (
